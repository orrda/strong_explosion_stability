...
def Y_sonic(q, l, eps=1e-5):
    ...
    def ode_system(xi, Y):
        ...
        return np.linalg.solve(MM(xi), NN @ Y)

    # Use a robust solver designed for potentially stiff systems
    sol = solve_ivp(ode_system, [1.0, last_xi], Y0, method='DOP853', rtol=1e-6, atol=1e-8)
...
for k in range(4):
    ax = axes[k // 3, k % 3]
    c = ax.pcolormesh(l_arr, s_arr, Y_arr[:, :, k], shading='auto', cmap='viridis')
...
# Plot Y_end
ax = axes[1, 1]
c_end = ax.pcolormesh(l_arr, s_arr, Y_B_arr, shading='auto', cmap='viridis')
